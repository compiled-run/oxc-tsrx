...
You need Node.js 20.19 or newer in the 20.x line, or 22.12 or newer. Node 21 and Node 22.0 through 22.11 are not supported. You do not need Rust, no package runs an install script, and no command downloads anything later. The prebuilt binaries cover macOS, Linux (both glibc and musl), and Windows, on x64 and arm64. There is no JavaScript or WebAssembly fallback, so a platform outside that list has to [build from source](https://oxc-tsrx-docs.vercel.app/guide/getting-started).
...
`setup` is the only extra step in the whole product. It is explicit, reversible with `oxc-tsrx remove`, never edits your `package.json`, and has to be run again after a clean dependency install, because `node_modules` is disposable. The [Vite+ guide](https://oxc-tsrx-docs.vercel.app/integrations/vite-plus) explains what it writes.
...
OXC for TSRX is an independent community project. It is not affiliated with, endorsed by, or a product of VoidZero or the OXC team. No OXC maintainer interest or endorsement is claimed. Canonical OXC is used as an unmodified upstream dependency. There is no source snapshot, no Cargo patch, no fork, no vendor tree, and no downstream patch queue. Nothing has been submitted to the OXC project and nothing is planned. The [upstream map](https://oxc-tsrx-docs.vercel.app/architecture/upstreaming-to-oxc) records what such a submission would have contained and why it is parked.
...
- You do not need Rust to use this package. You do need it to [build from source](https://oxc-tsrx-docs.vercel.app/guide/getting-started).
