...
// Leaf capability executor for this package's declared `format` capability.
//
...
// claims, so it formats exactly what it is handed and nothing else. Pointing
// the capability at a general host wrapper instead would make an adopting
// formatter re-enter itself without bound.
//
...
try {
  ...
  console.error(
    `oxc-tsrx-fmt: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exitCode = 2;
}
