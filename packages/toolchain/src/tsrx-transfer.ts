...
export const PROGRAM_BINARY_TRANSFER_MAGIC = 0x42525354;
export const PROGRAM_BINARY_TRANSFER_VERSION = 1;
const PROGRAM_TRANSFER_MAX_BYTES = 256 * 1024 * 1024;
...
const VALUE_INDEX_MASK = 0x3fffffff;
...
function applyFix(program, path) {
  ...
  for (const key of path) {
    if (
      ...
      invalid("fix path does not select a Program node");
    }
    node = node[key];
  }
  if (node === null || typeof node !== "object") {
    invalid("fix path does not end at a Program node");
  }
...
function parseMetadata(metadata, keyCount, scalarCount, fixCount) {
  ...
  if (!Array.isArray(decoded) || decoded.length !== 3) {
    invalid("metadata does not have exactly three tables");
  }
  const [keys, scalars, fixes] = decoded;
  if (!Array.isArray(keys) || keys.length !== keyCount) {
    invalid("key metadata count does not match the graph header");
  }
  for (const key of keys) {
    if (typeof key !== "string") invalid("key metadata contains a non-string");
  }
  if (!Array.isArray(scalars) || scalars.length !== scalarCount) {
    invalid("scalar metadata count does not match the graph header");
  }
  for (const scalar of scalars) {
    if (
      ...
      invalid("scalar metadata contains a non-scalar");
    }
  }
  if (!Array.isArray(fixes) || fixes.length !== fixCount) {
    invalid("fix metadata count does not match the graph header");
  }
  for (const path of fixes) {
    if (!Array.isArray(path)) invalid("fix metadata contains a non-path");
  }
  return { keys, scalars, fixes };
}
...
function parseBinaryProgram(payload) {
  ...
  if (
    words.byteLength > PROGRAM_TRANSFER_MAX_BYTES ||
    Buffer.byteLength(metadata, "utf8") >
      PROGRAM_TRANSFER_MAX_BYTES - words.byteLength
  ) {
    invalid("binary payload exceeds its bounded capacity");
  }
  if (words.length < PROGRAM_BINARY_HEADER_WORDS) {
    invalid("binary graph header is truncated");
  }
  if (words[0] !== PROGRAM_BINARY_TRANSFER_MAGIC) {
    invalid("binary graph magic does not match");
  }
  if (words[1] !== PROGRAM_BINARY_TRANSFER_VERSION) {
    invalid(`unsupported binary version ${String(words[1])}`);
  }
  ...
  if (!Number.isSafeInteger(expectedWords) || words.length !== expectedWords) {
    invalid("binary graph table lengths do not match the header");
  }
  if (rootTag !== OBJECT_TAG || rootIndex >= objectCount) {
    invalid("binary graph root is not a valid Program object");
  }

  const { keys, scalars, fixes } = parseMetadata(
    metadata,
    keyCount,
    scalarCount,
    fixCount,
  ...
  for (const owner of fieldOwners) {
    if (owner !== 1) invalid("binary object fields are unowned");
  }
  ...
  for (const owner of valueOwners) {
    if (owner !== 1) invalid("binary list values are unowned");
  }
  if (objects[rootIndex] === undefined) invalid("binary graph root is unused");
  ...
  for (let objectIndex = 0; objectIndex < objectCount; objectIndex += 1) {
    ...
    for (let fieldIndex = start; fieldIndex < start + count; fieldIndex += 1) {
      ...
      if (
        typeof key !== "string" ||
        (!(encodedKey & COMMON_KEY_FLAG) && encodedKey >= keyCount)
      ) {
        invalid("binary key index is out of range");
      }
      ...
      if (tag === SCALAR_TAG) {
        ...
        if (index >= objectCount || objects[index] === undefined) {
          invalid("binary object index is out of range or unused");
        }
        if (owners[index] !== 0) invalid("binary graph contains sharing or a cycle");
        owners[index] = 1;
        decoded = objects[index];
      } else if (tag === LIST_TAG) {
        if (index >= listCount || lists[index] === undefined) {
          invalid("binary list index is out of range or unused");
        }
        const owner = objectCount + index;
        if (owners[owner] !== 0) invalid("binary graph contains sharing or a cycle");
        owners[owner] = 1;
        decoded = lists[index];
      } else if (tag === INLINE_U32_TAG) {
        decoded = index;
      } else {
        invalid(`binary value tag ${String(tag)} is invalid`);
      }
      if (key === "__proto__") {
  ...
  for (let listIndex = 0; listIndex < listCount; listIndex += 1) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (tag === SCALAR_TAG) {
        ...
        if (valueIndex >= objectCount || objects[valueIndex] === undefined) {
          invalid("binary object index is out of range or unused");
        }
        ...
        if (valueIndex >= listCount || lists[valueIndex] === undefined) {
          invalid("binary list index is out of range or unused");
        }
  ...
  for (let index = 0; index < objectCount; index += 1) {
    if (owners[index] !== Number(objects[index] !== undefined)) {
      invalid("binary graph contains an unreachable container");
    }
  }
  for (let index = 0; index < listCount; index += 1) {
    if (owners[objectCount + index] !== Number(lists[index] !== undefined)) {
      invalid("binary graph contains an unreachable container");
    }
  }
...
function parseTrustedBinaryProgram(payload, stripTsrxCoreCompatDefaults = false) {
  ...
  const lists = new Array(listCount);
  for (let index = 0; index < objectCount; index += 1) {
    if (words[objectOffset + index * 2] !== UNUSED_RANGE) objects[index] = {};
  }
  for (let index = 0; index < listCount; index += 1) {
    if (words[listOffset + index * 2] !== UNUSED_RANGE) {
      lists[index] = new Array(words[listOffset + index * 2 + 1]);
    }
  }
  for (let listIndex = 0; listIndex < listCount; listIndex += 1) {
    if (lists[listIndex] === undefined) continue;
  ...
  for (let objectIndex = 0; objectIndex < objectCount; objectIndex += 1) {
    if (objects[objectIndex] === undefined) continue;
...
export function parseTsrxProgram(payload) {
  ...
  if (envelope.version !== PROGRAM_TRANSFER_VERSION) {
    invalid(`unsupported version ${String(envelope.version)}`);
  }
  if (envelope.node === null || typeof envelope.node !== "object") {
    invalid("Program is not an object");
  }
