  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        private: true,
        type: "module",
        devDependencies: { "oxc-tsrx": "0.2.1" },
      }, null, 2)}\n`,
    );
...
test("the canonical command names arbitrate from the project manifest alone", async () => {
  ...
  try {
    ...
    const plain = await project("plain", { "oxc-tsrx": "0.2.1" });
    ...
    const transitive = await project("transitive", { "oxc-tsrx": "0.2.1" }, { oxlint: officialOxlint });
    ...
    // tool.
    const missing = await project("missing", { "oxc-tsrx": "0.2.1", oxfmt: "0.44.0" });
...
async function providerFixture(temporary, name, { ownsLinterShim }) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "oxc-tsrx", version: "0.2.1", bin: { oxlint: "./bin/oxlint" } }, null, 2)}\n`,
  );
