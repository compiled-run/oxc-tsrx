...
test("the canonical command names arbitrate from the project manifest alone", async () => {
  ...
  try {
    ...
    const plain = await project("plain", { "oxc-tsrx": "0.1.1" });
    ...
    const transitive = await project("transitive", { "oxc-tsrx": "0.1.1" }, { oxlint: officialOxlint });
    ...
    // tool.
    const missing = await project("missing", { "oxc-tsrx": "0.1.1", oxfmt: "0.44.0" });
