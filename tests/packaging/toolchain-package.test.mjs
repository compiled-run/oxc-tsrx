  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        private: true,
        type: "module",
        devDependencies: { "oxc-tsrx": "0.1.0" },
      }, null, 2)}\n`,
    );
...
test("the canonical command names arbitrate from the project manifest alone", async () => {
  ...
  try {
    ...
    const plain = await project("plain", { "oxc-tsrx": "0.1.0" });
    ...
    const transitive = await project("transitive", { "oxc-tsrx": "0.1.0" }, { oxlint: officialOxlint });
    ...
    // tool.
    const missing = await project("missing", { "oxc-tsrx": "0.1.0", oxfmt: "0.44.0" });
