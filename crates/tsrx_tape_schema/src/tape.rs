...
impl ValueRef {
    ...
    pub const fn object(index: RecordIndex) -> Self {
        Self {
            index: index.into_raw(),
            length_or_tag: OBJECT_TAG,
        }
    }
    ...
    pub const fn as_inline_u32(self) -> Option<u32> {
        if self.length_or_tag == INLINE_U32_TAG {
            Some(self.index)
        } else {
            None
        }
    ...
    pub const fn as_object(self) -> Option<RecordIndex> {
        if self.length_or_tag == OBJECT_TAG {
            Some(RecordIndex::new(self.index))
        } else {
            None
        }
    }
    ...
    pub const fn as_list(self) -> Option<RecordIndex> {
        if self.length_or_tag == LIST_TAG {
            Some(RecordIndex::new(self.index))
        } else {
            None
        }
...
impl FlatTape {
    ...
    pub(crate) fn take_object_record_for_transfer(
        ...
        const VISITED: ObjectRecord = ObjectRecord {
            first_field: RecordIndex::NONE,
            field_count: u32::MAX,
        };
        let record = self
            .objects
            .get_mut(index_usize(object))
    ...
    pub(crate) fn take_list_record_for_transfer(
        ...
        const VISITED: ListRecord = ListRecord {
            first_value: RecordIndex::NONE,
            length: u32::MAX,
        };
        let record = self
            .lists
            .get_mut(index_usize(list))
            .ok_or(TapeBuildError::InvalidRecordIndex)?;
        if *record == VISITED {
            return Err(TapeBuildError::InvalidRecordIndex);
        }
    ...
    pub(crate) fn take_list_value_record_for_transfer(
        ...
        const VISITED: ListValueRecord = ListValueRecord {
            value: ValueRef::MISSING,
            next: RecordIndex::NONE,
        };
        let record = self
            .values
            .get_mut(index_usize(value))
            .ok_or(TapeBuildError::InvalidRecordIndex)?;
        if *record == VISITED {
            return Err(TapeBuildError::InvalidRecordIndex);
        }
    ...
    pub(crate) fn transfer_field_summary(
        ...
        for field in &self.fields {
            ...
            if field.value.as_inline_u32().is_some() {
                inline_u32_bytes = inline_u32_bytes
                    .checked_add(10)
                    .ok_or(TapeBuildError::CapacityOverflow)?;
            }
        }
    ...
    pub fn scalar_u32(&self, value: ValueRef) -> Option<u32> {
        value
            .as_inline_u32()
            .or_else(|| self.scalar(value)?.parse().ok())
    }
    ...
    pub fn push_key(&mut self, key: &str) -> Result<StringRange, TapeBuildError> {
        self.transfer_keys_require_escape |= key
            .bytes()
            .any(|byte| byte == b'"' || byte == b'\\' || byte <= 0x1f);
        ...
        while index < value.len() {
            ...
            match unit {
                value if (0xd800..=0xdfff).contains(&value) || value <= 0x1f => {
                    ...
                    self.scalars.push('u');
                    self.scalars
                        .push(char::from(HEX[usize::from((value >> 12) & 0x0f)]));
                    self.scalars
                        .push(char::from(HEX[usize::from((value >> 8) & 0x0f)]));
                    self.scalars
                        .push(char::from(HEX[usize::from((value >> 4) & 0x0f)]));
                    self.scalars
                        .push(char::from(HEX[usize::from(value & 0x0f)]));
                }
    ...
    pub fn push_field_record(
        ...
        self.transfer_key_bytes_upper = self
            .transfer_key_bytes_upper
            .saturating_add(record.key.length as usize);
        self.transfer_inline_u32_count_upper = self
            .transfer_inline_u32_count_upper
    ...
    pub fn append_field(
        ...
        let field = self.push_field_record(FieldRecord {
            key,
            value,
            next: RecordIndex::NONE,
        })?;
        ...
        self.objects[object_index].field_count = count
            .checked_add(1)
            .ok_or(TapeBuildError::CapacityOverflow)?;
        Ok(())
    }
    ...
    pub fn clear_fields(&mut self, object: RecordIndex) -> Result<(), TapeBuildError> {
        *self
            .objects
            .get_mut(index_usize(object))
            .ok_or(TapeBuildError::InvalidRecordIndex)? = ObjectRecord::default();
        Ok(())
    }
    ...
    pub fn insert_field_before(
        ...
        self.objects[object_index].field_count = record
            .field_count
            .checked_add(1)
            .ok_or(TapeBuildError::CapacityOverflow)?;
        Ok(())
    ...
    pub fn move_field_before(
        ...
        if self
            .fields
            .get(index_usize(field))
            .is_some_and(|record| record.next == before)
        {
            return Ok(());
        }
    ...
    pub fn remove_list_value(
        ...
        self.lists[list_index].length = record
            .length
            .checked_sub(1)
            .ok_or(TapeBuildError::InvalidRecordIndex)?;
        Ok(removed.value)
    ...
    pub fn remove_list_values(
        ...
        for (list_index, remove_count) in removal_counts.into_iter().enumerate() {
            ...
            self.lists[list_index] = ListRecord {
                first_value: first,
                length: kept,
            };
        }
    ...
    pub fn insert_list_values_after(
        ...
        for insertion in insertions {
            ...
            list_counts[list_index] = list_counts[list_index]
                .checked_add(1)
                .ok_or(TapeBuildError::CapacityOverflow)?;
        }
        ...
        for insertion in insertions {
            ...
            let inserted = self.push_list_value_record(ListValueRecord {
...
fn compact_objects(
    ...
    for (old_index, is_reachable) in reachable.objects.iter().copied().enumerate() {
        ...
        objects[index_usize(object_map[old_index])] = ObjectRecord {
            first_field: first,
            field_count: count,
        };
    }
...
fn compact_lists(
    ...
    for (old_index, is_reachable) in reachable.lists.iter().copied().enumerate() {
        ...
        lists[index_usize(list_map[old_index])] = ListRecord {
            first_value: first,
            length: count,
        };
    }
