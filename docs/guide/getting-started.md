...
| Editor misses `.tsrx`, `vp lint` sees it | Your editor needs `oxc.path.oxlint` in `.vscode/settings.json`. Under pnpm the official OXC extension finds Vite+'s own `oxlint`, which knows nothing about `.tsrx`; under npm the name can land nowhere at all, and a measured npm Vite+ tree had no `node_modules/.bin/oxlint` entry. `setup` writes that one line only when the extension would otherwise miss this package, leaves the rest of the file alone, and reports a value you set yourself rather than replacing it. The key is only read when the folder you **open** in VS Code is the folder holding that `.vscode/settings.json`: opened from a folder above it, the file is never read and the key does nothing. `setup` and `status` name every folder above your project that looks like a workspace root, and `setup --workspace-root <directory>` writes to the one you name. [The rest of the rules](/integrations/editor#when-the-key-does-not-take-effect). |
...
| Bare `oxlint` says `No files found to lint` | Under pnpm, `node_modules/.bin/oxlint` is Vite+'s own and cannot see `.tsrx` at all, and `setup` does not change that: it never rewrites anything in `node_modules/.bin`. (In a measured npm Vite+ tree there was no `.bin/oxlint` entry at all.) Your editor does not depend on that shim, because `setup` points it straight at `node_modules/oxc-tsrx/bin/oxlint` instead. Keep using `vp lint` and `vp fmt` anyway, because they read `vite.config.ts` while the bare commands read `.oxlintrc.json`, and here those are not the same file. |
